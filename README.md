...
\
The file, **docker-compose.yml**, run **two** MongoDB containers and **one** container with installed python and jupyter notebook. The first container, my_mongo, is used to store the data (use my_mongo:27017 to access stored data from jupyter notebook). The second one, mongo-express, is used for MongoDB Graphic User Interface (http://localhost:8081). The third container, c1, is used to run the code.
## Extraction
